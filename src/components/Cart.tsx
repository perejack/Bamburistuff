      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 mb-4">
        {items.map((item) => {
          ...
          return (
            <div 
              key={item.product.id} 
              className={`flex items-center gap-3 p-3 border border-gray-100 rounded-lg ${isNew ? 'animate-pulse bg-green-50' : 'bg-white'}`}
            >
              <img
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.product.category === 'cement' && (
                  <span className="inline-block bg-green-100 text-green-800 text-xs px-2 py-0.5 rounded-full mb-1">Premium</span>
                )}
